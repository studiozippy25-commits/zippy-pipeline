(function(){
  ...
  function defaultState(){
    var p=project()||{};
    return {version:3,mode:'preview',expertOpen:false,activeTab:'setup',setup:{format:p.defaultRatio||'16:9',world:'',stylePrefix:'',notes:''},assets:{character:{locked:false,evidence:0},location:{locked:false,evidence:0},prop:{locked:false,evidence:0},voice:{locked:false,evidence:0},behavior:{locked:false,evidence:0}},scenes:{},qc:{},logs:[],selectedScene:'',selectedShot:''};
  }
  ...
  function routePlan(shot,state){var refs=referenceCount(shot),textCue=hasTextCue(shot),image='gemini-image',video='seedance-2.5',post='없음',why=[];
    ...
    why.push('영상은 Seedance 2.5 전용 구조로 컴파일');
  ...
  function buildSeedancePrompt(shot,state){
    state=state||load();var p=project()||{}, people=exactPeople(shot), names=people.names.length?people.names.join(', '):'no visible character', props=propList(shot), g=sceneGeo(state,shot), action=physicalize(shot&&shot.desc||'hold the storyboard-defined physical action'), endState=shot&&shot.func||'the visible action reaches a clear readable completion';
    ...
    var visual=[String(state.setup.stylePrefix||'').trim(),String(p.quality||'').trim()].filter(Boolean).join('\n')||'Photorealistic live-action cinema with physical optics, natural skin and stable detail.';
    ...
    return [
      ...
      '[Scene]\n'+(g.space||shotLocation(shot)||'the location shown in @Image 1')+'. Anchor: '+(g.anchor||'the primary action area')+' at '+(g.anchorPosition||'the position established in @Image 1')+'. Lighting origin: '+(g.light||shot&&shot.light||'the single motivated source shown in @Image 1')+'.',
  ...
  function composeVideoPrompt(shot,legacy){return buildSeedancePrompt(shot,load());}
  ...
  function autoStyle(p){var q=String(p.quality||'').trim()||'naturalistic photorealistic live action';return [
    'Style: '+q,
  ...
  function renderPrompt(state){var shot=selectedShotFrom(state.selectedShot),id=shot&&shot.id||'';if(!shot)return '<div class="zv3-issue warning">스토리보드 쇼트가 없습니다.</div>';var report=analyze(shot,state);return '<div class="zv3-card"><h4>쇼트 선택</h4><select class="zv3-select" onchange="ZippyDirectorV3.selectPromptShot(this.value)">'+shotOptions(id)+'</select><div class="zv3-issues" style="margin-top:10px">'+issueHtml(report)+'</div><div class="zv3-actions"><button class="zv3-btn primary" onclick="ZippyDirectorV3.openShot(\''+js(id)+'\')">AI 감독 확인</button></div></div><div class="zv3-grid" style="margin-top:12px"><div class="zv3-card"><h4>스토리보드 이미지 프롬프트</h4><div class="zv3-code">'+esc(buildPrompt(shot,state))+'</div></div><div class="zv3-card"><h4>Seedance 2.5 영상 프롬프트</h4><div class="zv3-code">'+esc(buildSeedancePrompt(shot,state))+'</div></div></div>';}
  ...
  function render(){clearTimeout(mountTimer);mountTimer=setTimeout(function(){var el=document.getElementById('directorV3Mount'),assetEl=document.getElementById('directorV3AssetMount'),state=load(),body='';if(el){if(state.activeTab==='assets')body=renderAssets(state);else if(state.activeTab==='geo')body=renderGeo(state);else if(state.activeTab==='prompt')body=renderPrompt(state);else if(state.activeTab==='qc')body=renderQc(state);else body=renderSetup(state);el.innerHTML='<section class="zv3-shell">'+renderHeader(state)+(state.expertOpen?renderTabs(state)+'<div class="zv3-body">'+body+'</div>':'')+'</section>';}if(assetEl)assetEl.innerHTML=renderAssetInline(state);},0);}
  function cardControls(shot){var state=load(),report=analyze(shot,state),cls=report.errors.length?'bad':(report.warnings.length?'':'ok'),label=report.errors.length?'!':(report.warnings.length?String(report.warnings.length):'✓');return '<div class="zv3-shot-controls"><button class="zv3-shot-btn" onclick="event.stopPropagation();ZippyDirectorV3.openShot(\''+js(shot.id)+'\')">✨ AI 감독 확인</button><button class="zv3-shot-btn zv3-shot-dot '+cls+'" title="'+attr(report.errors.concat(report.warnings).join(' · ')||'통과')+'">'+label+'</button></div>';}
  ...
  function openShot(id){var shot=selectedShotFrom(id);if(!shot)return;var state=load(),report=analyze(shot,state),imagePrompt=buildPrompt(shot,state),videoPrompt=buildSeedancePrompt(shot,state);modal('AI 감독 확인 · '+id,'<div class="zv3-issues">'+issueHtml(report)+'</div><div class="zv3-actions"><button class="zv3-btn primary" onclick="navigator.clipboard.writeText(ZippyDirectorV3.buildSeedancePromptById(\''+js(id)+'\'))">Seedance 프롬프트 복사</button><button class="zv3-btn" onclick="navigator.clipboard.writeText(ZippyDirectorV3.buildPromptById(\''+js(id)+'\'))">이미지 프롬프트 복사</button></div><details class="zv3-technical"><summary>기술 상세 보기</summary><div class="zv3-grid"><div><div class="zv3-card">'+cameraHtml(report.camera)+'<h4>자동 모델 선택</h4><p>'+esc(report.route)+'</p></div><div class="zv3-code" style="margin-top:10px">'+esc(imagePrompt)+'</div></div><div class="zv3-code">'+esc(videoPrompt)+'</div></div></details>');}
