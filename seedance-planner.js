(function(){
  ...
  function gtiPrompt(shot){var imagePrompt='';try{if(window.ZippyDirectorV3)imagePrompt=window.ZippyDirectorV3.buildPrompt(shot)||'';}catch(e){}return 'SEEDANCE REFERENCE FRAME TASK\nCreate exactly one first frame for '+shot.id+'. Use documentary / v-log capture for live-action projects; retain the approved rendering medium for animation or illustrated projects. Use the supplied project assets as identity, face, wardrobe, location, and prop authorities. Preserve each asset only in its declared role. The composition must depict the instant immediately before the primary action begins, with readable geography and stable screen direction. This is one finished scene frame, not a character sheet, contact sheet, split screen, collage, or before-and-after layout. Do not invent a new face, outfit, location, prop, logo, or readable text.\n\n'+imagePrompt;}
