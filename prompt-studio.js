(function () {
  ...
  function syncProject() {
    if (project !== currentKey()) { if (rewriteController) rewriteController.abort(); project = currentKey(); items = []; items = [emptyShot()]; references = []; record = null; music = ''; sourcePrompt = ''; sourceName = ''; storageWarning = ''; return true; }
    return false;
  }
  ...
  function render() {
    ...
    var disabled = rewriteBusy ? ' disabled' : '';
  ...
  async function rewrite(value) {
    if (rewriteBusy) throw new Error('이미 분석 중입니다. 완료하거나 중지한 뒤 다시 실행하세요.');
    syncProject();
    var request = value ? clone(value) : { originalPrompt: sourcePrompt, model: model, projectKey: project, style: style };
    assertProject(request.projectKey);
    if (!request.originalPrompt || !request.originalPrompt.trim()) throw new Error('기존 프롬프트를 먼저 입력하세요.');
    if (value) sourceName = '';
    sourcePrompt = request.originalPrompt; model = request.model; record = null; rewriteBusy = true;
    ...
    try {
      ...
      var result = await window.ZippyPromptRewriter.rewrite(request, async function (instructions) {
        if (controller.signal.aborted) throw new Error('분석을 중지했습니다.');
        var response = await callLLM({ images: [], prompt: instructions, signal: controller.signal });
      ...
      if (project === request.projectKey && currentKey() === request.projectKey) { render(); status(message, failed); }
    }
  }
  async function loadSourceFile(file) {
    if (!file) return;
    if (rewriteBusy) throw new Error('분석 완료 후 파일을 가져오세요.');
  ...
  async function generate(value) {
    if (rewriteBusy) throw new Error('분석이 진행 중입니다. 완료 후 다시 구성하세요.');
  ...
  async function action(event) {
    ...
    try {
      if (rewriteBusy && target.dataset.action !== 'cancel-rewrite') throw new Error('분석 완료 후 조작하세요. 필요하면 분석 중지를 누르세요.');
      if (syncProject()) { render(); status('현재 프로젝트로 전환했습니다. 장면을 입력하세요.'); return; }
      if (target.dataset.model) { model = target.dataset.model; record = null; render(); return; }
      ...
      if (target.dataset.history) {
        ...
        record = clone(saved); model = record.model; items = clone(record.shots); references = clone(record.references || []); music = record.music || ''; style = record.style || ''; sourcePrompt = record.originalPrompt || ''; sourceName = record.sourceFileName || ''; render(); status('브라우저에 저장된 프롬프트를 열었습니다.'); return;
      }
  ...
  async function init() {
    ...
    root.onchange = function (event) { if (event.target.id === 'psSourceFile') loadSourceFile(event.target.files[0]).catch(function (error) { status('파일 읽기 실패 · ' + error.message, true); }); };
  ...
  async function applyCamera(value) {
    if (rewriteBusy) throw new Error('분석이 진행 중입니다. 완료 후 카메라를 적용하세요.');
