(function(){
  ...
  async function runFaceAudit(id){
    ...
    var authorities=selectFaceAuthorities(shot,refs);if(!authorities.length){toast('등장인물과 연결된 얼굴 원본을 찾지 못했습니다.','bad');return;}faceAuditBusy[id]=true;openShot(id);
    try{
      var characters=[];for(var i=0;i<authorities.length;i++){var a=authorities[i],out=await callLLM({images:[{b64:a.ref.b64,mime:a.ref.mime,_type:'face',_label:'APPROVED FACE AUTHORITY — '+a.name},{b64:frame.b64,mime:frame.mime,_type:'generated',_label:'GENERATED STORYBOARD FRAME'}],prompt:faceAuditPrompt(a.name,a.ref.label)}),parsed=parseJsonObject(out&&out.textOut||''),status=String(parsed.status||'unavailable').toLowerCase(),score=Math.max(0,Math.min(100,Number(parsed.score||0)));if(['pass','hold','fail','unavailable'].indexOf(status)===-1)status='unavailable';if(status!=='unavailable')status=score>=90?'pass':(score>=80?'hold':'fail');characters.push({label:a.name,authority:a.ref.label,score:score,status:status,visibility:String(parsed.visibility||''),confidence:Math.max(0,Math.min(100,Number(parsed.confidence||0))),mismatches:Array.isArray(parsed.mismatches)?parsed.mismatches.slice(0,8).map(String):[],summary:String(parsed.summary||'')});}
