...
원문 분석은 기존 파이프라인 `callLLM({images:[], prompt, signal})`을 사용한다. 상단 텍스트 모델 선택(Local / GPT / Gemini)에 따른 실제 호출이 발생한다. 이미지·영상 생성은 실행하지 않는다. 연결이 없거나 분석 JSON·대사 보존 검증이 실패하면 새 출력 없이 오류를 표시한다. API 호출 전 원문 입력은 브라우저 안에만 있으며 **분석·리라이팅** 실행 시 선택된 텍스트 모델로 전달된다.
